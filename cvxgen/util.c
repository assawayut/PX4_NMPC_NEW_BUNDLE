...
float toc(void) {
  ...
  tic_timestop = clock();
  printf("time: %8.2lf.\n", (long double)((tic_timestop - tic_timestart) / CLOCKS_PER_SEC));
  return (long double)((tic_timestop - tic_timestart) / CLOCKS_PER_SEC);
}
float tocq(void) {
  clock_t tic_timestop;
  tic_timestop = clock();
...
void printmatrix(char *name, double *A, int m, int n, int sparse) {
  ...
  for (i = 0; i < m; i++) {
    for (j = 0; j < n; j++)
      if ((sparse == 1) && (A[i+j*m] < 0.000001) && (A[i+j*m] > -0.000001))
        printf("         0");
      else
        printf("  % 9.4f", A[i+j*m]);
    printf(",\n");
...
float ran1(long*idum, int reset) {
  ...
  *idum = IA*(*idum-k*IQ)-IR*k;
  ...
  iy = iv[j];
  iv[j] = *idum;
  if ((temp=AM*iy)> (float)RNMX) return RNMX;
  else return temp;
}
/* Next function is from numerical recipes in C. */
float randn_internal(long *idum, int reset) {
  ...
  if (iset==0) {
    do {
      v1 = 2.0f*ran1(idum, reset)-1.0f;
      v2 = 2.0f*ran1(idum, reset)-1.0f;
      rsq = v1*v1+v2*v2;
    } while(rsq >= 1.0f || (rsq < 0.000001f && rsq > -0.000001f));
    fac = (float)sqrt(-2.0f*(float)log((float)rsq)/(float)rsq);
    gset = v1*fac;
    iset = 1;
    return v2*fac;
  } else {
